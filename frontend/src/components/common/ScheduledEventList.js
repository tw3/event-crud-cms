...
export class ScheduledEventList extends React.Component {
  ...
  render() {
    ...
    const per_page = 10;
    const pages = Math.ceil(scheduledEvents.length / per_page);
    const start_offset = (page - 1) * per_page;
    let start_count = 0;

    // show the list of events
    return ( // TODO create custom view rather than ugly table view
      <div>
        <Table bordered hover responsive striped>
          ...
          {scheduledEvents.map((scheduledEvent, index) => {
            if (index >= start_offset && start_count < per_page) {
              start_count++;
              return (
                <ScheduledEventListElement key={index} scheduledEvent={scheduledEvent} showDelete={this.showDelete} />
              );
            }
          })}
          </tbody>
        ...
        <Pagination className="events-pagination pull-right" bsSize="medium" maxButtons={10} first last next
          prev boundaryLinks items={pages} activePage={page} onSelect={this.changePage} />

        <ScheduledEventDeletePrompt show={this.state.bDeleteShow} scheduledEvent={this.state.deleteEvent}
